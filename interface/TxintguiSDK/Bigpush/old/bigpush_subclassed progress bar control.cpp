...
//FARPROC wpOrigProgressBarProc;
#define STRICT

#ifdef STRICT   
WNDPROC wpOrigProgressBarProc; 
#else   
FARPROC wpOrigProgressBarProc;
#endif

// Subclass procedure
LRESULT APIENTRY ProgressBarSubclassProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) 
{ 
    // not necessary???
    ...
    return CallWindowProc((FARPROC)wpOrigProgressBarProc, hWnd, uMsg, wParam, lParam); 
}
...
BOOL CALLBACK Tab2DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		wpOrigProgressBarProc = (WNDPROC)SetWindowLong( GetDlgItem(hWnd, IDC_PROGRESSVERT1), 
											GWL_WNDPROC, (LONG)ProgressBarSubclassProc );
