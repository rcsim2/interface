...
BOOL CALLBACK Tab3DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch(uMsg)
	{
		case WM_INITDIALOG:
			...
			SendMessage( GetDlgItem(hWnd, IDC_COMBO1), CB_SETCURSEL, 0, 0); g_bItemFree[4] = false;
			SendMessage( GetDlgItem(hWnd, IDC_COMBO2), CB_SETCURSEL, 1, 0); g_bItemFree[1] = false;
			SendMessage( GetDlgItem(hWnd, IDC_COMBO3), CB_SETCURSEL, 2, 0); g_bItemFree[2] = false;
			SendMessage( GetDlgItem(hWnd, IDC_COMBO4), CB_SETCURSEL, 3, 0); g_bItemFree[3] = false;
			SendMessage( GetDlgItem(hWnd, IDC_COMBO6), CB_SETCURSEL, 4, 0); g_bItemFree[0] = false;
...
*/
			...
			// track bar stuff
			for (i=0; i<10; i++) {
				SendMessage( GetDlgItem(hWnd, IDC_SLIDER1+i), TBM_SETRANGE, (WPARAM)TRUE, (LPARAM)MAKELONG(0, 100) );  
				SendMessage( GetDlgItem(hWnd, IDC_SLIDER1+i), TBM_SETPOS, (WPARAM)TRUE, (LPARAM)50 );
				
				SendMessage( GetDlgItem(hWnd, IDC_SLIDER1+i), TBM_SETTIC, (WPARAM)0, (LPARAM)(LONG)0 );
				SendMessage( GetDlgItem(hWnd, IDC_SLIDER1+i), TBM_SETTIC, (WPARAM)0, (LPARAM)(LONG)50 );
		...
		case WM_COMMAND:
			switch( LOWORD(wParam) )
			{
				case IDC_BUTTONCENTER:
					for (i=1; i<=10; i++) {
						SendMessage( GetDlgItem(hWnd, IDC_SLIDER1+(i-1)), TBM_SETPOS, (WPARAM)TRUE, (LPARAM)50 );
						g_iChSens[i] = 50;
						SetDlgItemText( hWnd, IDC_STATICSENS1+(i-1), "50%" );
					}
					break;
			...
			switch( HIWORD(wParam) )
			{
				...
				case CBN_SELENDOK:
					...
					for (i=0; i<10; i++) {
						if ( idComboBox != (IDC_COMBO1+i) && iNewItem != CB_ERR ) {
	
							if ( iNewItem == SendMessage( GetDlgItem(hWnd, IDC_COMBO1+i), CB_GETCURSEL, 0, 0 ) ) {
								SendMessage( GetDlgItem(hWnd, IDC_COMBO1+i), CB_SETCURSEL, -1, 0 );
							}
						}
					}
...
//     TOOLINFO structure. // Returns TRUE if successful or FALSE otherwise. 
