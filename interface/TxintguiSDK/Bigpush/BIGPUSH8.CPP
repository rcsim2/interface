	...
	case WM_COMMAND:
		switch( LOWORD(wParam) )
		{
			case IDC_BUTTONHORIZONTAL:
				if (g_hTabCurrent != NULL)
					DestroyWindow(g_hTabCurrent);
				// Kut: dit werkte niet omdat we een te kleine array hadden: arrays are evil, and so are pointers
				//g_hTabCurrent = CreateDialogIndirect(g_hInst,pTabPage[4],g_hTabControl,(DLGPROC)Tab5DlgProc);
				// This is much safer
				g_hTabCurrent = CreateDialog(g_hInst,MAKEINTRESOURCE(IDD_DIALOG5),GetParent(g_hTabControl),(DLGPROC)Tab5DlgProc); 
				ShowWindow(g_hTabCurrent, SW_SHOW);
				g_bHorizontal = true;
				break;
		}
		...
		case WM_COMMAND:
			switch( LOWORD(wParam) )
			{
				case IDC_BUTTONVERTICAL:
					...
					//g_hTabCurrent = CreateDialogIndirect(g_hInst,pTabPage[4],g_hTabControl,(DLGPROC)Tab5DlgProc);
					// This is much safer
					g_hTabCurrent = CreateDialog(g_hInst,MAKEINTRESOURCE(IDD_DIALOG2),GetParent(g_hTabControl),(DLGPROC)Tab2DlgProc); 
					ShowWindow(g_hTabCurrent, SW_SHOW);
					g_bHorizontal = false;
					break;
			}
